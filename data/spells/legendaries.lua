...
local spell = Card:createMinion(0, 'Annison Runebender', 7, 60, 0, 'All spells cost double the mana.', 60, 80);
	spell.onCast = function() this():addGlobalAura('MANACOST',function() if this().card.type == 'spell' then return this.card.manaCost else return 0 end end,false); end;
...
local spell = Card:createMinion(0,'Gwenyth Bladestar', 9, 60, 0, 'Deal 60 damage to all characters.', 60, 60);
	spell.onCast = function() local source=this(); game:damageForEach(character, 'NORMAL', 60, source.owner); end;
...
local spell = Card:createMinion(0, 'Thaurum Shadowmaw', 'DARK', 8, 120, 0, 'Consume a minion and gain its attack and health.', 10, 10);
	spell.onCast = function()  local thaurum = this(); local hp; local atk; game:chooseTarget(minion,function() hp = this():getHealth(); atk = this():getAttack(); thaurum:gain(atk, hp); this:destroy() end); end;